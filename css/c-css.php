 ...
 *   [if {!} {browser} {os}]
 *   [if {!} {browser_group} {os}]
 *   [if {!} {browser} {version} {os}]
 *   [if {!} {condition} {browser} {version} {os}]
 *   [if {!} {condition} {browser} {version} {os}]
 * Examples:
...
interface ifCcss
{
	...
	public function fnSetUserOS ();	//iwm
...
class ccss implements ifCcss
{
	...
	public function fnOutputHeader ()
	{
		...
		echo " * User Agent:    ".$this->os.PHP_EOL;
		echo " */\n";
	...
	public function fnSetUserOS() {		//iwm
		if ( preg_match( '/Mac/i', $this->sUserAgent, $aUserAgent ) ) {
			$this->os = "mac";
		}
		else if ( preg_match( '/Windows/i', $this->sUserAgent, $aUserAgent ) ) {
			$this->os = "win";
		}
		else {
			$this->os = "other";
	...
	{
		while (strpos($sImport,"  ") > 0) {
			$sImport = str_replace("  "," ",$sImport);
		}
	...
	private function _fnCheckCC ( $sCC )
	{
		...
		/*
		 *	grab the last parm and see if it's an OS
		 *  strip it out if it is - easiest way to hack it in
		 */
		if (array_key_exists($aCC[count($aCC)-1],array("win"=>"win","mac"=>"mac","other"=>"other"))) {
			$os = $aCC[count($aCC)-1];
			array_splice($aCC,count($aCC)-1);
		}
		else $os = "";
