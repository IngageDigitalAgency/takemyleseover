...
class lookups extends Backend {

	private $m_tree = 'code_folders';
	private $m_content = 'code_lookups';
	private $m_perrow = 5;
	
	public function __construct() {
		$this->M_DIR = 'backend/modules/lookups/';
		$this->setTemplates(
			array(
				'deleteItem'=>$this->M_DIR.'forms/deleteItem.html',
				'deleteItemResult'=>$this->M_DIR.'forms/deleteItemResult.html',
				'main'=>$this->M_DIR.'lookups.html',
				'form'=>$this->M_DIR.'forms/form.html',
				'showContentTree'=>$this->M_DIR.'forms/contenttree.html',
				'lookupsInfo'=>$this->M_DIR.'forms/lookupsInfo.html',
		...
		$this->setFields(array(
			...
			'header'=>array(),
			'addContent'=>array(
				'options'=>array('method'=>'post','action'=>'/modit/ajax/addContent/lookups','database'=>false),
				'id'=>array('name'=>'id','type'=>'tag','database'=>false),
				'type'=>array('name'=>'type','type'=>'select','sql'=>'select tablename,title from code_folders order by title','required'=>true),
				'code'=>array('name'=>'code','type'=>'input','required'=>true),
				'value'=>array('name'=>'value','type'=>'input','required'=>true),
				'extra'=>array('name'=>'extra','type'=>'textarea','required'=>false),
				'sort'=>array('name'=>'sort','type'=>'textfield','required'=>true,'value'=>0),
				'submit'=>array('type'=>'submitbutton','database'=>false,'value'=>'Save'),
				'p_id'=>array('name'=>'p_id','type'=>'hidden','database'=>false),
			...
			'folderProperties' => array(
				'options'=>array(
					'action'=>'/modit/lookups/showPageProperties',
					'method'=>'post'
				),
		...
		if (count($_POST) > 0 && array_key_exists('showPageProperties',$_POST)) {
			...
			if ($valid) {
				...
				if ($data['id'] == 0) {
					...
					$data['id'] = $mptt->add($_POST['p_id'],999,array('title'=>$_POST['title'],'tablename'=>preg_replace('#[^a-z0-9]#i', '_',strtolower($_POST['title']))));
				} 
				else {
					//
				...
				if ($status) {
					...
					if ($this->isAjax()) {
						...
						return $this->ajaxReturn(array(
								'status'=>'true',
								'html'=>'',
								'url'=>'/modit/lookups?p_id='.$data['id']
						));
					}
				} else {
	...
	function showPageContent($fromMain = false) {
		...
		if ($p_id > 0 && $data = $this->fetchSingle(sprintf('select * from %s where id = %d',$this->m_tree,$p_id))) {
			...
			$count = $this->fetchScalar(sprintf('select count(n.id) from %s n, %s t where t.id = %d and n.type = t.tablename', $this->m_content, $this->m_tree, $_REQUEST['p_id']));
			...
			$sql = sprintf('select n.*, 0 as j_id, t.title from %s n, %s t where t.id = %d and n.type = t.tablename order by %s %s limit %d,%d',  $this->m_content, $this->m_tree, $_REQUEST['p_id'],$sortby, $sortorder, $start,$perPage);
	...
	function showSearchForm($fromMain = false,$msg = '') {
		...
		if (count($_POST) == 0)
			if (array_key_exists('formData',$_SESSION) && array_key_exists('lookupsSearchForm', $_SESSION['formData']))
				$_POST = $_SESSION['formData']['lookupsSearchForm'];
			else
		...
		if (count($_POST) > 0 && array_key_exists('showSearchForm',$_POST)) {
			$form->addData($_POST);
			if ($form->validate()) {
				$_SESSION['formData']['lookupsSearchForm'] = $form->getAllData();
				$srch = array();
				...
				if (count($srch) > 0) {
					...
					$perPage = $this->m_perrow;
					if (array_key_exists('pager',$_REQUEST)) $perPage = $_REQUEST['pager'];
					$count = $this->fetchScalar(sprintf('select count(n.id) from %s n, %s t where 1=1 and %s and n.type = t.tablename', $this->m_content, $this->m_tree, implode(' and ',$srch)));
					...
					$sql = sprintf('select n.*,t.title, 0 as j_id from %s n, %s t where n.type = t.tablename and %s order by %s %s limit %d,%d',
	...
	function addContent($fromMain = false) {
		...
		if (!(array_key_exists('a_id',$_REQUEST) && $_REQUEST['a_id'] > 0 && $data = $this->fetchSingle(sprintf('select * from %s where id = %d', $this->m_content, $_REQUEST['a_id'])))) {
			$data['id'] = 0;
			$data['type'] = $this->fetchScalar(sprintf('select tablename from %s where id = %d',$this->m_tree,$data['p_id'])); 
		}
		else $data['p_id'] = array_key_exists('p_id',$_REQUEST) ? $_REQUEST['p_id'] : 0;
		//
		//	access levels
		//
		$level = $this->getAccessLevel();
		...
		if (count($_POST) > 0 && array_key_exists('addContent',$_POST)) {
			...
			if ($form->validate()) {
				...
				if ($status = $stmt->execute()) {
					...
					if ($data['id'] == 0)
						return $this->ajaxReturn(array('status' => 'true','url' => sprintf('/modit/lookups?p_id=%d',$_POST['p_id'])));
					else {
	...
	function getHeader() {
		...
		if (count($_POST) > 0 && array_key_exists('showSearchForm',$_POST))
			...
			if (array_key_exists('formData',$_SESSION) && array_key_exists('lookupsSearchForm', $_SESSION['formData']))
				$form->addData($_SESSION['formData']['lookupsSearchForm']);
		return $form->show();
	}
