...
function addItem() {
	clearMessages();
	$.ajax({
		url: '/modit/ajax/showPageProperties/modules',
		data: {'id': 0},
...
function moveFolder() {
	...
	setTimeout(function() {loadTree('modules',id)},100);
}
...
function getInfo(lnk) {
	...
	var id = lnk.split("_");
	$.ajax({
		url: '/modit/ajax/getFolderInfo/modules',
		data: {p_id: id[1]},
		success: function(data,textStatus,jqXHR) {
			try {
				obj = eval('('+data+')');
...
function loadContent(cId) {
	$.ajax({
		url: '/modit/ajax/showPageContent/modules',
		data: {p_id : cId},
		method: 'post',
		dataType: "html",
		success: function(data,textStatus,jqXHR) {
...
function deleteContent(cId) {
	if (confirm("Are you sure you want to delete this item?")) {
		$.ajax({
			url: '/modit/ajax/deleteContent/modules',
			data: {'p_id':cId},
			success: function(data,textStatus,jqXHR) {
				try {
					obj = eval('('+data+')');
...
function loadSearchForm(id) {
	$.ajax({
		url: '/modit/ajax/showSearchForm/modules',
		data: {'p_id':id},
		success: function(data,textStatus,jqXHR) {
			try {
				obj = eval('('+data+')');
...
var editArticle = function(m_id) {
	$.ajax({
		url: '/modit/ajax/addContent/modules',
		data: {'m_id': m_id },
		success: function(data,textStatus,jqXHR) {
			try {
				obj = eval('('+data+')');
...
function deleteArticle(id) {
	if (confirm("Are you sure?")) {
		$.ajax({
			url: '/modit/ajax/deleteArticle/modules',
			data: {'j_id':id},
			type:'post',
			success: function(data,textStatus,jqXHR) {
				try {
					obj = eval('('+data+')');
					if (obj.status == 'true')
						formCheck("showSearchForm","/modit/ajax/showSearchForm/modules","middleContent");
					else
						if (obj.messages)
							showError(obj.messages);
				} catch(err) {
...
function getOptionsRow() {
	var retValue = $.ajax({
		url: '/modit/ajax/getOptionsRow/modules',
		async: false,
		type:'post'
	});
	try {
		var obj = eval('('+retValue.responseText+')');
		$('#optionsTable tbody').append(obj.html);
...
function addSubtemplate(s_id,o_id) {
	if (o_id <= 0) {
		alert('The Template must be saved first');
		return;
	}
	var retValue = $.ajax({
		url: '/modit/ajax/addSubtemplate/modules',
		data: {fetemplate_id:o_id,s_id:s_id},
		async:false,
		type:'post'
	});
	try {
...
function loadSubtemplates(p_id) {
	var retValue = $.ajax({
		url:'/modit/ajax/buildTemplates/modules',
		data:{'fetemplate_id':p_id},
		async:false,
		type:'post'
	});
	try {
		var obj = eval('('+retValue.responseText+')');
		if (obj.status == 'true') {
			$('#subtemplateListing tbody').html("").append(obj.html);
		}
		if (obj.code) eval(obj.code);
		if (obj.messages && obj.messages.length > 0) showPopupError(obj.messages);
	}
...
function deleteTemplate(s_id,p_id) {
	if (confirm('Are you sure you want to delete this record?')) {
		var retValue = $.ajax({
			url:'/modit/ajax/deleteSubtemplate/modules',
			data:{s_id:s_id,p_id:p_id,'deleteSubtemplateForm':1},
			async:false,
			type:'post'
		});
		try {
...
function addPlacement(s_id,o_id) {
	if (o_id <= 0) {
		alert('The Template must be saved first');
		return;
	}
	var retValue = $.ajax({
		url: '/modit/ajax/addPlacement/modules',
		data: {fetemplate_id:o_id,s_id:s_id},
		async:false,
		type:'post'
	});
...
function loadPlacements(p_id) {
	var retValue = $.ajax({
		url:'/modit/ajax/buildPlacements/modules',
		data:{'fetemplate_id':p_id},
		async:false,
		type:'post'
	});
	try {
		var obj = eval('('+retValue.responseText+')');
		if (obj.status == 'true') {
			$('#formplacementListing tbody').html("").append(obj.html);
		}
		if (obj.code) eval(obj.code);
		if (obj.messages && obj.messages.length > 0) showPopupError(obj.messages);
	}
...
function deletePlacement(s_id,p_id) {
	if (confirm('Are you sure you want to delete this record?')) {
		var retValue = $.ajax({
			url:'/modit/ajax/deletePlacement/modules',
			data:{s_id:s_id,p_id:p_id,'deletePlacementForm':1},
			async:false,
			type:'post'
		});
		try {
...
function editHtml(obj) {
	html = $(obj).siblings("input");
	$.ajax({
		url: "/modit/ajax/editHtml/modules",
		data: {file:html.val(),class:$("#module_id").val()},
