...
function addItem() {
	clearMessages();
	$.ajax({
		url: '/modit/ajax/showPageProperties/advert',
		data: {'id': 0},
...
function getInfo(lnk) {
	...
	var id = lnk.split("_");
	$.ajax({
		url: '/modit/ajax/getFolderInfo/advert',
		data: {p_id: id[1]},
		success: function(data,textStatus,jqXHR) {
			try {
				obj = eval('('+data+')');
...
function loadContent(cId) {
	$.ajax({
		url: '/modit/ajax/showPageContent/advert',
		data: {p_id : cId},
		method: 'post',
		dataType: "html",
		success: function(data,textStatus,jqXHR) {
...
function deleteContent(cId) {
	if (confirm("Are you sure you want to delete this item?")) {
		$.ajax({
			url: '/modit/ajax/deleteContent/advert',
			data: {'p_id':cId},
			success: function(data,textStatus,jqXHR) {
				try {
					obj = eval('('+data+')');
...
function loadSearchForm(id) {
	$.ajax({
		url: '/modit/ajax/showSearchForm/advert',
		data: {'p_id':id},
		success: function(data,textStatus,jqXHR) {
			try {
				obj = eval('('+data+')');
...
var editArticle = function(a_id) {
	$.ajax({
		url: '/modit/ajax/addContent/advert',
		data: {'a_id': a_id },
		success: function(data,textStatus,jqXHR) {
			try {
				obj = eval('('+data+')');
...
var formCheck_add = function (frmId,f_url,el) {
	...
	$("#".concat(frmId, ' #toList > li')).each(function(idx,el) {
		...
		o.val(ids[1]);
		o.appendTo($("#".concat(frmId, ' #advertDestFolders')));
	});
	formCheck(frmId,f_url,el);
}

var paginationDNU = function (pnum, url, el, obj) {
	var p_id = $('#advertFolderId')[0].innerHTML
	...
	if (dest == 'tree') {
		...
		if (srcId[0] > 0 && destId[1] > 0) {
			$("#dialog-confirm" ).dialog({
				...
				modal: true,
				buttons: {
					"Copy the ad": function() {
						$.ajax({
							url: '/modit/ajax/moveArticle/advert',
...
function deleteArticle(a_id,j_id) {
	$.ajax({
		url: '/modit/ajax/deleteArticle/advert',
...
function initSearch() {
	if ($('#search-tabs').length > 0) {
		$('#pager').change(function() {
			formCheck("searchForm","/modit/ajax/showSearchForm/advert","middleContent");
		});
...
function reSort(src,dest) {
	...
	data = $.ajax({
		url: '/modit/ajax/moveArticle/advert',
		data: {'src':src[1],'dest':dest[1],'type':'advert'},
		async:false
	});
	try {
		obj = eval('('+data.responseText+')');
		if (obj.status == 'true') {
			formCheck("showFolderContent","/modit/ajax/showPageContent/advert","middleContent");
		}
		if (obj.messages)
			showError(obj.messages);
		if (obj.code && obj.code.length > 0)
		...
		$( '#articleList tbody').sortable({
			...
			update: function( event, ui) {
				if(!dropped) {
					...
					advertDrop(idx[1],curLoc,null,'news');
				}
				dropped = false;
			}
		});
