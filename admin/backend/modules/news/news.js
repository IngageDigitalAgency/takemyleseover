...
function addItem() {
	clearMessages();
	$.ajax({
		url: '/modit/ajax/showPageProperties/news',
		data: {'id': 0},
...
function getInfo(lnk) {
	...
	var id = lnk.split("_");
	$.ajax({
		url: '/modit/ajax/getFolderInfo/news',
		data: {p_id: id[1]},
		success: function(data,textStatus,jqXHR) {
			try {
				obj = eval('('+data+')');
...
function loadContent(cId) {
	$.ajax({
		url: '/modit/ajax/showPageContent/news',
		data: {p_id : cId},
		method: 'post',
		dataType: "html",
		success: function(data,textStatus,jqXHR) {
		...
		$( '#articleList tbody').sortable({
			...
			update: function( event, ui) {
				if(!dropped) {
					...
					newsDrop(idx[1],curLoc,null,'news');
				}
				dropped = false;
			}
		});
...
function deleteContent(cId) {
	...
	if (confirm("Are you sure you want to delete this item?")) {
		$.ajax({
			url: '/modit/ajax/deleteContent/news',
			data: {'p_id':cId},
			success: function(data,textStatus,jqXHR) {
				try {
					obj = eval('('+data+')');
					if (obj.status == "true") {
						document.location = '/modit/news'.concat('?rd=',Math.random());	// force a refresh
					}
					else {
						if (obj.code && obj.code.length > 0) eval(obj.code);
...
function loadSearchForm(id) {
	$.ajax({
		url: '/modit/ajax/showSearchForm/news',
		data: {'p_id':id},
		success: function(data,textStatus,jqXHR) {
			try {
				obj = eval('('+data+')');
...
var editArticle = function(a_id) {
	$.ajax({
		url: '/modit/ajax/addContent/news',
		data: {'a_id': a_id },
		success: function(data,textStatus,jqXHR) {
			try {
				obj = eval('('+data+')');
...
var formCheck_add = function (frmId,f_url,el) {
	...
	$("#".concat(frmId, ' #toList > li')).each(function(idx,el) {
		...
		o.val(ids[1]);
		o.appendTo($("#".concat(frmId, ' #newsDestFolders')));
	});
	formCheck(frmId,f_url,el);
}

var paginationDNU = function (pnum, url, el, obj) {
	var p_id = $('#newsFolderId')[0].innerHTML
...
function newsDrop(obj,evt,el,dest) {
	...
	if (dest == 'tree') {
			...
			$("#dialog-confirm" ).dialog({
				...
				buttons: {
					"Copy the article": function() {
						$.ajax({
							url: '/modit/ajax/moveArticle/news',
		...
		$.ajax({
			url: '/modit/ajax/moveArticle/news',
			data: {'src': obj, 'dest': evt, 'type':'news'},
			success: function(data,textStatus,jqXHR) {
				try {
					obj = eval('('+data+')');
					if (obj.status != 'true') {
...
function deleteArticle(a_id,j_id) {
	$.ajax({
		url: '/modit/ajax/deleteArticle/news',
...
function initSearch() {
	if ($('#search-tabs').length > 0) {
		$('#pager').change(function() {
			formCheck("searchForm","/modit/ajax/showSearchForm/news","middleContent");
		});
...
function reSort(src,dest) {
	...
	data = $.ajax({
		url: '/modit/ajax/moveArticle/news',
		data: {'src':src[1],'dest':dest[1],'type':'news'},
		async:false
	});
	try {
		obj = eval('('+data.responseText+')');
		if (obj.status == 'true') {
			formCheck("showFolderContent","/modit/ajax/showPageContent/news","middleContent");
		}
		if (obj.messages)
			showError(obj.messages);
		if (obj.code && obj.code.length > 0)
