...
function addItem() {
	clearMessages();
	$.ajax({
		url: '/modit/ajax/showPageProperties/search',
		data: {'id': 0},
...
function getInfo(lnk) {
	...
	var id = lnk.split("_");
	$.ajax({
		url: '/modit/ajax/getFolderInfo/search',
		data: {p_id: id[1]},
		success: function(data,textStatus,jqXHR) {
			try {
				obj = eval('('+data+')');
...
function loadContent(cId) {
	$.ajax({
		url: '/modit/ajax/showPageContent/search',
		data: {p_id : cId},
		method: 'post',
		dataType: "html",
		success: function(data,textStatus,jqXHR) {
		...
		$( '#articleList tbody').sortable({
			...
			update: function( event, ui) {
				if(!dropped) {
					...
					searchDrop(idx[1],curLoc,null,'search');
				}
				dropped = false;
			}
		});
...
function deleteContent(cId) {
	...
	if (confirm("Are you sure you want to delete this item?")) {
		$.ajax({
			url: '/modit/ajax/deleteContent/search',
			data: {'p_id':cId},
			success: function(data,textStatus,jqXHR) {
				try {
					obj = eval('('+data+')');
					if (obj.status == "true") {
						document.location = '/modit/search'.concat('?rd=',Math.random());	// force a refresh
					}
					else {
						if (obj.code && obj.code.length > 0) eval(obj.code);
...
function loadSearchForm(id) {
	$.ajax({
		url: '/modit/ajax/showSearchForm/search',
		data: {'p_id':id},
		success: function(data,textStatus,jqXHR) {
			try {
				obj = eval('('+data+')');
...
var editArticle = function(a_id,p_id) {
	...
	$.ajax({
		url: '/modit/ajax/addContent/search',
...
function searchDrop(obj,evt,el,dest) {
	...
	if (dest == 'tree') {
			...
			$("#dialog-confirm" ).dialog({
				...
				buttons: {
					"Copy the article": function() {
						$.ajax({
							url: '/modit/ajax/moveArticle/search',
		...
		$.ajax({
			url: '/modit/ajax/moveArticle/search',
			data: {'src': obj, 'dest': evt, 'type':'search'},
			success: function(data,textStatus,jqXHR) {
				try {
					obj = eval('('+data+')');
					if (obj.status != 'true') {
...
function deleteArticle(a_id,j_id) {
	$.ajax({
		url: '/modit/ajax/deleteArticle/search',
...
function initSearch() {
	if ($('#search-tabs').length > 0) {
		$('#pager').change(function() {
			formCheck("searchForm","/modit/ajax/showSearchForm/search","middleContent");
		});
...
function reSort(src,dest) {
	...
	data = $.ajax({
		url: '/modit/ajax/moveArticle/search',
		data: {'src':src[1],'dest':dest[1],'type':'search'},
		async:false
	});
	try {
		obj = eval('('+data.responseText+')');
		if (obj.status == 'true') {
			formCheck("showFolderContent","/modit/ajax/showPageContent/search","middleContent");
		}
		if (obj.messages)
			showError(obj.messages);
		if (obj.code && obj.code.length > 0)
