...
function addItem() {
	clearMessages();
	$.ajax({
		url: '/modit/ajax/showPageProperties/subscribers',
		data: {'id': 0},
...
function moveFolder() {
	id = moveIt('subscriber_folders',this);
	setTimeout(function() {
		loadTree('subscribers',id)
		makeDroppable();
	},100);
}
...
function getInfo(lnk) {
	...
	var id = lnk.split("_");
	$.ajax({
		url: '/modit/ajax/getFolderInfo/subscribers',
		data: {p_id: id[1]},
		success: function(data,textStatus,jqXHR) {
			try {
				obj = eval('('+data+')');
...
function loadContent(cId) {
	$.ajax({
		url: '/modit/ajax/showPageContent/subscribers',
		data: {p_id : cId},
		method: 'post',
		dataType: "html",
		success: function(data,textStatus,jqXHR) {
...
function deleteContent(cId) {
	if (confirm("Are you sure you want to delete this item?")) {
		$.ajax({
			url: '/modit/ajax/deleteContent/subscribers',
			data: {'p_id':cId},
			success: function(data,textStatus,jqXHR) {
				try {
					obj = eval('('+data+')');
...
function loadSearchForm(id) {
	$.ajax({
		url: '/modit/ajax/showSearchForm/subscribers',
		data: {'p_id':id},
		success: function(data,textStatus,jqXHR) {
			try {
				obj = eval('('+data+')');
...
var editArticle = function(a_id) {
	$.ajax({
		url: '/modit/ajax/addContent/subscribers',
		data: {'a_id': a_id },
		success: function(data,textStatus,jqXHR) {
			try {
				obj = eval('('+data+')');
...
var formCheck_add = function (frmId,f_url,el) {
	...
	$("#".concat(frmId, ' #toList > li')).each(function(idx,el) {
		...
		o.val(ids[1]);
		o.appendTo($("#".concat(frmId, ' #subscriberDestFolders')));
	});
...
function subscribersDrop(obj,evt,el,dest) {
	...
	if (dest == 'tree') {
		...
		if (srcId[0] > 0 && destId[1] > 0) {
			$("#dialog-confirm" ).dialog({
				...
				modal: true,
				buttons: {
					"Copy the subscriber": function() {
						$.ajax({
							url: '/modit/ajax/moveArticle/subscribers',
...
function loadActiveFolder() {
	if ($('#contentTree a.active').length == 0)
		$('#search_form').submit();
	else
		$('#contentTree a.active').each(function (idx,el) {
...
function deleteArticle(a_id,j_id) {
	$.ajax({
		url: '/modit/ajax/deleteArticle/subscribers',
...
function initSearch() {
	if ($('#search-tabs').length > 0) {
		$('#pager').change(function() {
			formCheck("searchForm","/modit/ajax/showSearchForm/subscribers","middleContent");
		});
...
function reSort(src,dest) {
	...
	data = $.ajax({
		url: '/modit/ajax/moveArticle/subscribers',
		data: {'src':src[1],'dest':dest[1],'type':'subscribers'},
		async:false
	});
	try {
		obj = eval('('+data.responseText+')');
		if (obj.status == 'true') {
			formCheck("showFolderContent","/modit/ajax/showPageContent/subscribers","middleContent");
		}
		if (obj.messages)
			showError(obj.messages);
		if (obj.code && obj.code.length > 0)
		...
		$( '#articleList tbody').sortable({
			...
			update: function( event, ui) {
				if(!dropped) {
					...
					subscribersDrop(idx[1],curLoc,null,'news');
				}
				dropped = false;
			}
		});
...
function makeDroppable() {
	$('#contentTree ol.ui-sortable li .wrapper').droppable({
		...
		drop: function( event, ui ) {
			dropped = true;
			subscribersDrop(this,event,ui,'tree');
		}
	});
}
