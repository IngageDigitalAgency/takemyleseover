...
class leasing extends Backend {
	...
	public function __construct() {
		...
		$this->setFields(array(
			...
			'addContent'=>array(
				...
				'contact_id'=>array('type'=>'select','sql'=>'select id, concat(lastname,", ",firstname) from members where id = %%contact_id%%','value'=>0,'id'=>'contact_id'),
	...
	function showForm() {
		...
		$tmp = $form->buildForm($this->getFields('form'));
	...
	function showPageContent($fromMain = false) {
		...
		if ($p_id > 0 && $data = $this->fetchSingle(sprintf('select * from %s where id = %d',$this->m_tree,$p_id))) {
			...
			$sql = sprintf('select a.*, f.id as j_id, c1.name as make, c2.model, m1.firstname, m1.lastname, m2.firstname as cfname, m2.lastname as clname, m2.id as c_id from %s a left join %s f on a.id = f.lease_id left join members m2 on m2.id = a.contact_id, lease_make c1, lease_model c2, members m1 where m1.id = a.owner_id and c1.id = a.make and c2.id = a.model and a.deleted = 0 and f.folder_id = %d order by %s %s limit %d,%d',  $this->m_content, $this->m_junction, $_REQUEST['p_id'],$sortby, $sortorder, $start,$perPage);
		...
		if (count($_POST) > 0 && array_key_exists('showSearchForm',$_POST)) {
			...
			if ($form->validate()) {
				...
				if (count($srch) > 0) {
					...
					$sql = sprintf('select n.*, c1.name as make, c2.model, m1.firstname, m1.lastname, 0 as j_id, m2.firstname as cfname, m2.lastname as clname, m2.id as c_id from %s n left join members m2 on m2.id = n.contact_id, lease_make c1, lease_model c2, members m1 where m1.id = n.owner_id and c1.id = n.make and c2.id = n.model and %s order by %s %s limit %d,%d',
	...
	function addContent($fromMain = false) {
		...
		if (!($rec = $this->fetchSingle(sprintf("select * from lease_model where id = %d",$data["model"]))))
		...
		if (count($_POST) > 0 && array_key_exists(__FUNCTION__,$_POST)) {
			...
			if ($status) {
				...
				if ($status = $stmt->execute()) {
					...
					$destFolders = array_key_exists("destFolders", $_REQUEST) ? $_POST['destFolders'] : array(0);
