...
function addItem() {
	clearMessages();
	$.ajax({
		url: '/modit/ajax/showPageProperties/rss',
		data: {'id': 0},
...
function getInfo(lnk) {
	...
	var id = lnk.split("_");
	$.ajax({
		url: '/modit/ajax/getFolderInfo/rss',
		data: {p_id: id[1]},
		success: function(data,textStatus,jqXHR) {
			try {
				obj = eval('('+data+')');
...
function loadContent(cId) {
	$.ajax({
		url: '/modit/ajax/showPageContent/rss',
		data: {p_id : cId},
		method: 'post',
		dataType: "html",
		success: function(data,textStatus,jqXHR) {
...
function deleteContent(cId) {
	...
	if (confirm("Are you sure you want to delete this item?")) {
		$.ajax({
			url: '/modit/ajax/deleteContent/rss',
			data: {'p_id':cId},
			success: function(data,textStatus,jqXHR) {
				try {
					obj = eval('('+data+')');
					if (obj.status == "true") {
						document.location = '/modit/rss'.concat('?rd=',Math.random());	// force a refresh
					}
					else {
						if (obj.code && obj.code.length > 0) eval(obj.code);
...
function loadSearchForm(id) {
	$.ajax({
		url: '/modit/ajax/showSearchForm/rss',
		data: {'p_id':id},
		success: function(data,textStatus,jqXHR) {
			try {
				obj = eval('('+data+')');
...
var formCheck_add = function (frmId,f_url,el) {
	...
	$("#".concat(frmId, ' #toList > li')).each(function(idx,el) {
		...
		o.val(ids[1]);
		o.appendTo($("#".concat(frmId, ' #rssDestFolders')));
	});
	formCheck(frmId,f_url,el);
}

var paginationDNU = function (pnum, url, el, obj) {
	var p_id = $('#rssFolderId')[0].innerHTML
...
function rssDrop(obj,evt,el,dest) {
	...
	if (dest == 'tree') {
			...
			$("#dialog-confirm" ).dialog({
				...
				buttons: {
					"Copy the article": function() {
						$.ajax({
							url: '/modit/ajax/moveArticle/rss',
		...
		$.ajax({
			url: '/modit/ajax/moveArticle/rss',
			data: {'src': obj, 'dest': evt, 'type':'rss'},
			success: function(data,textStatus,jqXHR) {
				try {
					obj = eval('('+data+')');
					if (obj.status != 'true') {
...
function deleteArticle(a_id,j_id) {
	$.ajax({
		url: '/modit/ajax/deleteArticle/rss',
...
function initSearch() {
	if ($('#search-tabs').length > 0) {
		$('#pager').change(function() {
			formCheck("searchForm","/modit/ajax/showSearchForm/rss","middleContent");
		});
...
function reSort(src,dest) {
	...
	data = $.ajax({
		url: '/modit/ajax/moveArticle/rss',
		data: {'src':src[1],'dest':dest[1],'type':'rss'},
		async:false
	});
	try {
		obj = eval('('+data.responseText+')');
		if (obj.status == 'true') {
			formCheck("showFolderContent","/modit/ajax/showPageContent/rss","middleContent");
		}
		if (obj.messages)
			showError(obj.messages);
		if (obj.code && obj.code.length > 0)
...
getAuthor = function() {
$('td#author_id .chosen-search input').autocomplete({
	source:function(request,response) {
		$.ajax({
			url:'/modit/ajax/getAuthor/rss',
			data:{'s':request.term,'m':$("select[name=author_id]").val()},
			async:true,
			context:this,
