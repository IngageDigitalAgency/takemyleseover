function addItem() {
	clearMessages();
	$.ajax({
		url: '/modit/ajax/showPageProperties/blog',
		data: {'id': 0},
...
function getInfo(lnk) {
	...
	var id = lnk.split("_");
	$.ajax({
		url: '/modit/ajax/getFolderInfo/blog',
		data: {p_id: id[1]},
		success: function(data,textStatus,jqXHR) {
			try {
				obj = eval('('+data+')');
...
function loadContent(cId) {
	clearMessages();
	$.ajax({
		url: '/modit/ajax/showPageContent/blog',
		data: {p_id : cId},
		method: 'post',
		dataType: "html",
		success: function(data,textStatus,jqXHR) {
...
function deleteContent(cId) {
	if (confirm("Are you sure you want to delete this item?")) {
		$.ajax({
			url: '/modit/ajax/deleteContent/blog',
			data: {'p_id':cId},
			success: function(data,textStatus,jqXHR) {
				try {
					obj = eval('('+data+')');
...
function loadSearchForm(id) {
	clearMessages();
	$.ajax({
		url: '/modit/ajax/showSearchForm/blog',
		data: {'p_id':id},
		success: function(data,textStatus,jqXHR) {
			try {
				obj = eval('('+data+')');
...
var editArticle = function(a_id) {
	clearMessages();
	$.ajax({
		url: '/modit/ajax/addContent/blog',
		data: {'a_id': a_id },
		success: function(data,textStatus,jqXHR) {
			try {
				obj = eval('('+data+')');
...
var formCheck_add = function (frmId,f_url,el) {
	...
	$("#".concat(frmId, ' #toList > li')).each(function(idx,el) {
		...
		o.val(ids[1]);
		o.appendTo($("#".concat(frmId, ' #blogDestFolders')));
	});
	formCheck(frmId,f_url,el);
}

var paginationDNU = function (pnum, url, el, obj) {
	var p_id = $('#blogFolderId')[0].innerHTML
...
function blogDrop(obj,evt,el,dest) {
	...
	if (dest == 'tree') {
			...
			$("#dialog-confirm" ).dialog({
				...
				buttons: {
					"Copy the article": function() {
						$.ajax({
							url: '/modit/ajax/moveArticle/blog',
		...
		$.ajax({
			url: '/modit/ajax/moveArticle/blog',
			data: {'src': obj, 'dest': evt, 'type':'blog'},
			success: function(data,textStatus,jqXHR) {
				try {
					obj = eval('('+data+')');
					if (obj.status != 'true') {
...
function deleteArticle(b_id,j_id) {
	$.ajax({
		url: '/modit/ajax/deleteArticle/blog',
		data: {'j_id':j_id,'b_id':b_id},
		success: function(data,textStatus,jqXHR) {
			try {
				obj = eval('('+data+')');
...
function loadComments(id) {
	clearMessages();
	$.ajax({
		url: '/modit/ajax/loadComments/blog',
		data: {'b_id': id},
		success: function(data,textStatus,jqXHR) {
			try {
				obj = eval('('+data+')');
...
function editComment(id) {
	clearMessages();
	$.ajax({
		url: '/modit/ajax/editComment/blog',
		data: {'c_id': id},
		success: function(data,textStatus,jqXHR) {
			try {
				obj = eval('('+data+')');
...
function reloadComments(obj) {
	var id = $('#addContent input[name=a_id]')[0].value;
	loadComments(id);
}
...
function deleteComment(id) {
	if (confirm('Are you sure you want to delete the comment?')) {
		$.ajax({
			url: '/modit/ajax/deleteComment/blog',
			data: {'c_id':id},
			success: function(data,textStatus,jqXHR) {
				try {
					obj = eval('('+data+')');
...
function initSearch() {
	if ($('#search-tabs').length > 0) {
		$('#pager').change(function() {
			formCheck("searchForm","/modit/ajax/showSearchForm/blog","middleContent");
		});
		...
		$( '#articleList tbody').sortable({
			...
			update: function( event, ui) {
				if(!dropped) {
					...
					blogDrop(idx[1],curLoc,null,'news');
				}
				dropped = false;
			}
		});
...
function loadFromEdit() {
	...
	if ($('#search_form').length > 0)
		formCheck("search_form","/modit/ajax/showSearchForm/blog","middleContent");
}
