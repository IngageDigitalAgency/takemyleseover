 ...
 */
    ...
    try {
      ...
      if (empty($this->Body)) {
        throw new phpmailerException($this->Lang('empty_message'), self::STOP_CRITICAL);
			...
			$this->myCommon->logMessage(__FUNCTION__,"call CreateHeader", 1);
      $this->MIMEHeader = $this->CreateHeader();

			$this->myCommon->logMessage(__FUNCTION__,"call CreateBody", 1);
      ...
      $this->SentMIMEMessage = sprintf("%s%s\r\n\r\n%s",$this->MIMEHeader,$mailHeader,$this->MIMEBody);
			$this->myCommon->logMessage(__FUNCTION__,sprintf("returning this [%s]", print_r($this,true)), 1);
      return true;
  ...
  protected function PostSend() {
    try {
      // Choose the mailer and send through it
			$this->myCommon->logMessage(__FUNCTION__,sprintf("starting with [%s]", $this->Mailer), 1);
      switch($this->Mailer) {
        case 'sendmail':
        ...
        default:
          return $this->MailSend($this->MIMEHeader, $this->MIMEBody);
			...
			$this->myCommon->logMessage(__FUNCTION__,sprintf("ending with [%s]", $this->Mailer), 1);
  ...
  protected function MailSend($header, $body) {
  	$this->myCommon->logMessage(__FUNCTION__,sprintf("header [%s] body [%s]", print_r($header,true), print_r($body,true)), 1);
    $toArr = array();
  ...
  protected function SmtpSend($header, $body) {
    ...
    $bad_rcpt = array();
		...
		$this->myCommon->logMessage(__FUNCTION__,sprintf("smtpConnect"), 1);
    if(!$this->SmtpConnect()) {
      throw new phpmailerException($this->Lang('smtp_connect_failed'), self::STOP_CRITICAL);
    }
		$this->myCommon->logMessage(__FUNCTION__,sprintf("back from smtpConnect smtp [%s]", print_r($this->smtp,true)), 1);
    ...
    // Attempt to send attach all recipients
		$this->myCommon->logMessage(__FUNCTION__,sprintf("start receipients"), 1);
    foreach($this->to as $to) {
      if (!$this->smtp->Recipient($to[0])) {
		...
		$this->myCommon->logMessage(__FUNCTION__,sprintf("end receipients"), 1);
    foreach($this->cc as $cc) {
      if (!$this->smtp->Recipient($cc[0])) {
		...
		$this->myCommon->logMessage(__FUNCTION__,sprintf("end cc"), 1);
    foreach($this->bcc as $bcc) {
      if (!$this->smtp->Recipient($bcc[0])) {
		...
		$this->myCommon->logMessage(__FUNCTION__,sprintf("end bcc"), 1);
    ...
    if (count($bad_rcpt) > 0 ) { //Create error message for any bad addresses
      ...
      throw new phpmailerException($this->Lang('recipients_failed') . $badaddresses);
		...
		$this->myCommon->logMessage(__FUNCTION__,sprintf("send Header [%s] body [%s]", $header, $body), 1);
    if(!$this->smtp->Data($header . $body)) {
      throw new phpmailerException($this->Lang('data_not_accepted'), self::STOP_CRITICAL);
    }
		$this->myCommon->logMessage(__FUNCTION__,sprintf("send done"), 1);
