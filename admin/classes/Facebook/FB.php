...
class FB extends Facebook {
	...
	public function getPosts($objectId=null,$fields=array()) {
		return $this->_apiGet('posts', $objectId, $fields);
	}
	 ...
	 * @return object|array
	 */
	protected function _apiGet($key=null, $objectId=null, $fields = array()) {
		try {
			$apiKey = $key !== null ?   ('/' . $key) : '';
			$_objectId = $objectId !== false ? ('/' . $this->getObjectId($objectId)) : '';
			if (is_array($fields) && count($fields) > 0)
				$cmd = $_objectId . $apiKey. sprintf("?fields=%s",implode(",",$fields));
			else
				$cmd = $_objectId . $apiKey;
			$this->innerResult = $this->api($cmd);
		} catch (FacebookApiException $e) {
		  $this->innerResult = $e;
          return $e;
        }
