...
class googleAnalytics extends Common {
	...
	public function __construct($params = array())
	{
		...
		$this->logMessage("__construct",sprintf("params [%s]",print_r($params,true)),2);
	...
	public function getData($aProperties = array()) {
		...
		$inDimensions = array_key_exists('dimensions',$aProperties) ? explode(",",$aProperties['dimensions']) : array();
		$inMetrics = array_key_exists('metrics',$aProperties) ? explode(",",$aProperties['metrics']) : array();
		$myDimensions = array();
		foreach ($oEntries as $oEntry)
		{
			...
			$oDimensions = $oEntry->getElementsByTagName('dimension');
			$tmp = array();
			for($x = 0; $x < count($inDimensions); $x++) {
				$tmp[] = $oDimensions->item($x)->getAttribute('value');
			}
			$mTmp = array();
			for($x = 0; $x < count($inMetrics); $x++) {
				$mTmp[] = $oMetric->item($x)->getAttribute('value');
			}
			$myDimensions[implode('|',$tmp)] = implode('|',$mTmp);
		}
		// cache the results (if caching is true)
		$this->setCache($sUrl, $myDimensions);
	 ...
	 */
	public function getUrl($sUrl, $aPost = array(), $aHeader = array())
	{
		if (count($aPost) > 0)
		{
			// build POST query
	...
	public function getGeneric($type,$metric = array('visits')) {
		$types = array();
		foreach($type as $key) {
			$types[] = 'ga:'.$key;
		}
		$metrics = array();
		foreach($metric as $key) {
			$metrics[] = 'ga:'.$key;
		}
		if (count($type) > 0) {
			$aData = $this->getData(array(
				'dimensions' => implode(',',$types),
