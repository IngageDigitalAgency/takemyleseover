...
class Snoopy extends Common
{
			...
			case "http":
				...
				if ($this->_connect($fp)) {
					...
					if ($this->_framedepth < $this->maxframes && count($this->_frameurls) > 0) {
						...
						foreach($frameurls as $xx=>$frameurl) {
	...
	{
		$this->logMessage(__FUNCTION__,sprintf("uri [%s] this [%s] vars [%s] files [%s]", print_r($URI,true), print_r($this,true), print_r($formvars,true), print_r($formfiles,true)),2);
		unset($postdata);
		...
		switch (strtolower($URI_PARTS["scheme"])) {
			case "https":
				if (!extension_loaded('openssl')) {
					$this->logMessage(__FUNCTION__,sprintf("openssl doesn't seem to be installed"),1,true);
					trigger_error("openssl extension required for HTTPS", E_USER_ERROR);
					exit;
				}
				$this->port = 443;
	...
	function _httprequest($url, $fp, $URI, $http_method, $content_type = "", $body = "")
	{
		...
		$this->logMessage(__FUNCTION__,sprintf("actual transmission header [%s] body [%s]", $headers, $body), 3);
		...
		while ($currentHeader = fgets($fp, $this->_maxlinelen)) {
			$this->logMessage(__FUNCTION__,sprintf("returned header [%s]", $currentHeader),3);
		...
		do {
			...
			$this->logMessage(__FUNCTION__,sprintf("returned data [%s]", $_data),3);
			if (strlen($_data) == 0) {
				break;
			}
			$results .= $_data;
		...
		$this->logMessage(__FUNCTION__,sprintf("raw results [%s]", print_r($results,true)),3);
		...
		// no framed content
			...
			$this->results = $results;
				$this->logMessage(__FUNCTION__,sprintf("results [%s] this [%s]", print_r($results,true), print_r($this,true)),2);
		return $this;
	}
	...
	function _connect(&$fp)
	{
		...
		$this->logMessage(__FUNCTION__,sprintf("open fp [%s] host [%s] port [%s] errno [%s] errmsg [%s] context [%s]", print_r($fp,true), $host, $port, $errno, $errmsg, print_r($context,true)),3);
...
//				while (list($key, $val) = each($formvars)) {
				...
				if (substr($postdata, strlen($postdata) - 1, 1) == "&") {
					$this->logMessage(__FUNCTION__,sprintf("stripping & from [%s] to [%s]", $postdata, substr($postdata, 0, strlen($postdata) - 2)),1);
					$postdata = substr($postdata, 0, strlen($postdata) - 1);	// strip trailing &
				}
				break;
			...
			case "multipart/form-data":
				...
				foreach($formfiles as $field_name=>$file_name) {
					...
					foreach($file_names as $xx=>$file_name) {
